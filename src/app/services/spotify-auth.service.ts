...
export class SpotifyAuthService {
  ...
  refreshToken(): Observable<AccessToken> {
    ...
    return this.requestToken(payload).pipe(
      tap((token) => this.storeToken(token, storedToken.refresh_token)),
   ...
   * @param previousRefreshToken The existing refresh token to preserve when Spotify does not return one.
   * @private
   */
  private storeToken(token: AccessToken, previousRefreshToken?: string): void {
  ...
  private isTokenExpired(token: StoredAccessToken): boolean {
    return !token.expires_at || Date.now() >= token.expires_at;
  }
